...
#define print_dp_he(j, DP_HE, dp_sn, dp_beg, dp_end, score_t) { \
    int i;                                                      \
    printf("index: %d\t", j);                                   \
    SIMDi *dp_h = DP_HE + j * 2 * dp_sn, *dp_e = dp_h + dp_sn;  \
    score_t *_dp_h = (score_t*)dp_h, *_dp_e = (score_t*)dp_e;   \
...
#define print_simd(s, str, score_t) {                           \
    int i; score_t *a = (score_t*)s;                            \
    printf("%s\t", str);                                        \
    for (i = 0; i < SIMDTotalBytes / sizeof(score_t); ++i) {    \
        printf("%d\t", a[i]);                                   \
    } printf("\n");                                             \
}

/*
static inline void simd_bpoa_backtrack(SIMDi *DP_HE, int dp_sn, score_t match, score_t mis, score_t gap_e, int **pre_index, int *pre_n, SIMDi *backtrack_z, int d_sn, int best_i, int best_j, bpoa_graph_t *graph, uint8_t *query, int *n_cigar, bpoa_cigar_t **graph_cigar) {
    int i, j, k, pre_i;
...
#define simd_bpoa_banded_global_align_sequence_with_graph_core(graph, query, qlen, bpt, n_cigar, graph_cigar, sp, score_t,   \
    ...
    {   /* DP loop */                                                                                                       \
        ...
        for (i = 0; i <= dp_end_sn[0]; ++i) SIMDStorei(dp_e+i, min_inf);                                                    \
    ...
    { /* backtrack from best score */                                                                                       \
        if (n_cigar && graph_cigar) {                                                                                       \
            simd_bpoa_backtrack(score_t, DP_HE, dp_sn, (score_t)bpt->match, (score_t)bpt->mismatch, (score_t)bpt->gap_ext, pre_index, pre_n, backtrack_z, d_sn, best_i, best_j, graph, query, n_cigar, graph_cigar);                                                              \
        }                                                                                                                   \
    }                                                                                                                       \
                                                                                                                            \
    { /* free variables */                                                                                                  \
...
int simd8_bpoa_banded_global_align_sequence_with_graph_core(bpoa_graph_t *graph, uint8_t *query, int qlen, bpoa_para_t *bpt, int *n_cigar, bpoa_cigar_t **graph_cigar, SIMD_para_t sp) {
    int **pre_index, *pre_n, pre_i;
    int target_node_n = graph->node_n - 2, matrix_row_n = graph->node_n, matrix_col_n = qlen + 1;
    int i, j, k, w, *dp_beg, *dp_beg_sn, *dp_end, *dp_end_sn, node_id, index_i, q_i;
    int pn , qp_sn,d_sn, dp_sn, size; // pn: value per SIMDi, qp_sn/dp_sn/d_sn: segmented length
    ...
    SIMDi *backtrack_z, *z; // backtrack cell: f<<4|e<<2|h, MATCH:0, DELETION:1, INSERTION:2
    SIMDi *hd, fd, ed, hm, he, hf, ee, em, ff, fm; uint8_t m0=0x0, e1=0x1, f2=0x2;
    // int8_t is based on score_n 8/16/32
    int8_t *_dp_h, *_dp_e, *_dp_f, best_score = sp.inf_min, inf_min = sp.inf_min; int best_i=0, best_j=0;
    int *mat = bpt->mat, gap_open = bpt->gap_open, gap_ext = bpt->gap_ext, gap_OE = bpt->gap_open + bpt->gap_ext;
    ...
    {   // allocate memory 
        ...
        min_inf = SIMDSetOnei8(inf_min); gap_e = SIMDSetOnei8(bpt->gap_ext), gap_oe = SIMDSetOnei8(bpt->gap_open + bpt->gap_ext);
        if (graph_cigar && n_cigar) {
            hd = (SIMDi*)SIMDMalloc(d_sn * (target_node_n + 1) * size, size);
            backtrack_z = hd + d_sn;

            // for backtrack
            hm = SIMDSetOnei8(m0 << HOP_OFF_SET), he = SIMDSetOnei8(e1 << HOP_OFF_SET), hf = SIMDSetOnei8(f2 << HOP_OFF_SET);
            em = SIMDSetOnei8(m0 << EOP_OFF_SET), ee = SIMDSetOnei8(e1 << EOP_OFF_SET), fm = SIMDSetOnei8(m0 << FOP_OFF_SET), ff = SIMDSetOnei8(f2 << FOP_OFF_SET);
        }
    ...
    {   // generate the query profile
        for (k = 0; k < bpt->m; ++k) { // SIMD parallelization
            int *p = &mat[k * bpt->m];
            int8_t *_qp = (int8_t*)(qp + k * qp_sn);
            _qp[0] = 0; 
            for (j = 0; j < qlen; ++j) _qp[j+1] = (int8_t)p[query[j]];
        }
    ...
    {   // DP loop
        ...
        dp_h = DP_HE, dp_e = dp_h + dp_sn;
        _dp_h = (int8_t*)dp_h, _dp_e = (int8_t*)dp_e;
        for (i = 0; i <= dp_end_sn[0]; ++i) dp_e[i] = min_inf;
        for (i = 1; i <= dp_end[0]; ++i) { // no SIMD parallelization
            _dp_h[i] = -gap_open - gap_ext * i;
        } _dp_h[0] = 0; _dp_e[0] = -(gap_OE);
        ...
        for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {
            ...
            SIMDi *q = qp + graph->node[node_id].base * qp_sn, a, b, c, d;
            dp_h = DP_HE + index_i * 2 * dp_sn, dp_e = dp_h + dp_sn;
            _dp_h = (int8_t*)dp_h, _dp_e = (int8_t*)dp_e, _dp_f = (int8_t*)dp_f;
            ...
            for (i = 0; i < pre_n[index_i]; ++i) {
                ...
                // set M from (pre_i, q_i-1)
                _end_sn = (pre_end + pn) / pn-1;
                SIMDi first = SIMDShiftRight(min_inf, SIMDTotalBytes-SIMDShiftOneNi8);
                for (sn_i = pre_beg_sn; sn_i <= _end_sn; ++sn_i) { // SIMD parallelization
                    a = SIMDLoadi(pre_dp_h+sn_i); b = SIMDLoadi(dp_h+sn_i);
                    SIMDi remain = SIMDShiftLeft(a, SIMDShiftOneNi8);
                    SIMDStorei(dp_h+sn_i, SIMDMaxi8(SIMDOri(first, remain), b));
                    first = SIMDShiftRight(a, SIMDTotalBytes-SIMDShiftOneNi8);
                }

                for (sn_i = pre_beg_sn; sn_i <= pre_end_sn; ++sn_i) { // SIMD parallelization
                    a = SIMDLoadi(pre_dp_e+sn_i); b = SIMDLoadi(dp_e+sn_i);
                    SIMDStorei(dp_e+sn_i, SIMDMaxi8(a, b));
                }
            }
            ...
            if (n_cigar && graph_cigar) {
                ...
                // compare M, E, and F
                for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { // SIMD parallelization
                    a = SIMDLoadi(dp_h+sn_i); b = SIMDLoadi(q+sn_i); a = SIMDAddi8(a, b);
                    c = SIMDLoadi(dp_e+sn_i); d = SIMDLoadi(hd+sn_i); 
                    SIMDGetIfGreateri8(d, a, c, a, he, hm);
                    SIMDStorei(hd+sn_i, d); SIMDStorei(dp_h+sn_i, a);
                }
                ...
                for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { // SIMD parallelization
                    a = SIMDLoadi(dp_h+sn_i); b = SIMDLoadi(dp_f+sn_i); c = SIMDLoadi(dp_e+sn_i); d = SIMDLoadi(hd+sn_i);
                    // h, hd for current cell
                    SIMDGetIfGreateri8(d, a, b, a, hf, d);
                    SIMDStorei(dp_h+sn_i, a);

                    // e, ed for next cell
                    tmp_e = SIMDSubi8(a, gap_oe);
                    c = SIMDSubi8(c, gap_e);
                    if (gap_open) {
                        SIMDGetIfGreateri8(ed, c, c, tmp_e, ee, em);
                        SIMDStorei(dp_e+sn_i, c);
                    } else {
                        SIMDSetIfEquali8(ed, d, hm, em, ee);
                        SIMDStorei(dp_e+sn_i, tmp_e);
                    }
                    ...
                    b = SIMDSubi8(b, gap_e);
                    if (gap_open) {
                        SIMDSetIfGreateri8(fd, b, tmp_e, ff, fm);
                    } else {
                        SIMDSetIfEquali8(fd, d, hm, fm ,ff);
                    }
                    SIMDStorei(z+sn_i, SIMDOri(SIMDOri(d, ed), fd));
                }
            } else {
                // compare M, E, and F
                for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { // SIMD parallelization
                    a = SIMDLoadi(dp_h+sn_i); 
                    b = SIMDLoadi(q+sn_i);
                    c = SIMDLoadi(dp_e+sn_i);
                    a = SIMDAddi8(a, b);  // q[0] == 0
                    SIMDStorei(dp_h+sn_i, SIMDMaxi8(a, c));
                }
                ...
                for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { // SIMD parallelization
                    ...
                    tmp_e = SIMDSubi8(a, gap_oe);
                    c = SIMDSubi8(c, gap_e);
                    SIMDStorei(dp_e+sn_i, SIMDMaxi8(c, tmp_e));
                }
            }
        }
    }
    ...
    { // backtrack from best score
        if (n_cigar && graph_cigar) {
            simd_bpoa_backtrack(int8_t, DP_HE, dp_sn, (int8_t)bpt->match, (int8_t)bpt->mismatch, (int8_t)bpt->gap_ext, pre_index, pre_n, backtrack_z, d_sn, best_i, best_j, graph, query, n_cigar, graph_cigar);
        }
    }
...
int simd_bpoa_banded_global_align_sequence_with_graph(bpoa_graph_t *graph, uint8_t *query, int qlen, bpoa_para_t *bpt, int *n_cigar, bpoa_cigar_t **graph_cigar) {
    ...
    int _best_score;
    if (max_score <= 127) { // DP_H/E/F: 8  bits
        ...
        return _best_score;
    } else if (max_score <= 32767) { // DP_H/E/F: 16 bits
