...
#define score_t int16_t
void simd_lg_backtrack(SIMDi *DP_H, int dp_sn, int match, int mis, int gap_e, int **pre_index, int *pre_n,
        int start_i, int start_j, int best_i,int best_j, int qlen, abpoa_graph_t *graph, uint8_t *query, int *n_cigar, abpoa_cigar_t **graph_cigar) {                    
    int i, j, k, pre_i;
    SIMDi *dp_h, *pre_dp_h; score_t *_dp_h, *_pre_dp_h;                                                                      
    int n_c = 0, s, m_c = 0, hit, id;                                                                                        
    abpoa_cigar_t *cigar = 0;                                                                                                
...
/*int simd16_abpoa_lg_global_align_sequence_with_graph_core(abpoa_graph_t *graph, uint8_t *query, int qlen, abpoa_para_t *abpt, int *n_cigar, abpoa_cigar_t **graph_cigar, SIMD_para_t sp) {
    ...
    {   // generate the query profile
        for (i = 0; i < qp_sn * abpt->m; ++i) 
            qp[i] = min_inf;
            //SIMDStorei(&qp[i], min_inf);
        for (k = 0; k < abpt->m; ++k) { // SIMD parallelization
            int *p = &mat[k * abpt->m];
            int16_t *_qp = (int16_t*)(qp + k * qp_sn);
            _qp[0] = 0; 
        ...
        for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {
            ...
            for (i = 0; i < pre_n[index_i]; ++i) {
                ...
                for (sn_i = _beg_sn; sn_i <= _end_sn; ++sn_i) { // SIMD parallelization
                    ...
                    //SIMDi remain = SIMDShiftLeft(a, SIMDShiftOneNi16);
                    h = SIMDAddi16(SIMDOri(first, remain), q[sn_i]); e = SIMDSubi16(pre_dp_h[sn_i], gap_e);
                    //h = SIMDAddi16(SIMDOri(first, remain), c); e = SIMDSubi16(a, gap_e);
                    dp_h[sn_i] = SIMDMaxi16(h, SIMDMaxi16(e, dp_h[sn_i]));
            ...
            for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { // SIMD parallelization
                dp_h[sn_i] = SIMDMaxi16(dp_f[sn_i], dp_h[sn_i]);
            ...
            } else if (abpt->bw >= 0) { // set min/max_rank for next nodes
                int max_out_rank = graph->node_id_to_max_rank[node_id] + 1;
                int min_out_rank = graph->node_id_to_min_rank[node_id] + 1;
                for (i = 0; i < graph->node[node_id].out_edge_n; ++i) {
                    int out_node_id = graph->node[node_id].out_id[i];
...
void simd_get_m_e(int beg, int end_sn, int *pre_n, int **pre_index, int index_i, SIMDi *DP_H, SIMDi *dp_h, SIMDi *q, int *dp_beg, int *dp_beg_sn, int *dp_end, int *dp_end_sn, int dp_sn, int pn, SIMDi min_inf, SIMDi gap_e) {
    int i, pre_i, sn_i, pre_beg, pre_beg_sn, pre_end, pre_end_sn, _beg_sn, _end_sn;
    SIMDi *pre_dp_h, a, b, c, h, e;
    // get max m and e
    for (i = 0; i < pre_n[index_i]; ++i) {
        pre_i = pre_index[index_i][i];
        pre_dp_h = DP_H+pre_i * dp_sn;
...
void simd_get_f(int beg, int end, SIMDi *dp_h, SIMDi *dp_f, int gap_ext) {
    int q_i;
    int16_t *_dp_h = (int16_t*)dp_h;
    int16_t *_dp_f = (int16_t*)dp_f;
    for (q_i = beg+1; q_i <= end; ++q_i) {
        _dp_f[q_i] = MAX_OF_TWO(_dp_h[q_i-1], _dp_f[q_i-1]) - gap_ext;
    }
}

void simd_get_H(int beg_sn, int beg, int end_sn, int end, SIMDi *dp_h, SIMDi *dp_f) {
    int i, sn_i;
    int16_t *_dp_h = (int16_t*)dp_h, *_dp_f = (int16_t*)dp_f;
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { // SIMD parallelization
        dp_h[sn_i] = SIMDMaxi16(dp_f[sn_i], dp_h[sn_i]);
        //SIMDStorei(&dp_h[sn_i], SIMDMaxi16(dp_f[sn_i], dp_h[sn_i]));
    }
...
void simd_dp_loop(int index_i, int *dp_beg, int *dp_beg_sn, int *dp_end, int *dp_end_sn, int beg_sn, int beg, int end_sn, int end, int dp_sn, int pn, SIMDi min_inf, int gap_ext, int *pre_n, int **pre_index, SIMDi *dp_h, SIMDi *DP_H, SIMDi *dp_f, SIMDi *q, SIMDi gap_e) {
    int i, sn_i, pre_i, pre_beg, pre_beg_sn, pre_end, pre_end_sn, _beg_sn, _end_sn, q_i;
    SIMDi *pre_dp_h, h, e;
    int16_t *_dp_h, *_dp_f;
    // loop query
    // init h, e                       
    ...
    simd_get_m_e(beg, end_sn, pre_n, pre_index, index_i, DP_H, dp_h, q, dp_beg, dp_beg_sn, dp_end, dp_end_sn, dp_sn, pn, min_inf, gap_e);
    simd_get_f(beg, end, dp_h, dp_f, gap_ext);
    simd_get_H(beg_sn, beg, end_sn, end, dp_h, dp_f);
}

void simd_ada_rank(abpoa_graph_t *graph, SIMDi *DP_H, int *pre_n, int **pre_index, int max_i, int gap_ext, int match, int mismatch, uint8_t *query, int node_id, int index_i, int dp_sn) {
    // determine max pre_i, then determin current rank
    int s, max_pre_i=-1, max_pre_id, k, pre_i, i;
    int16_t *_dp_h, *_pre_dp_h; SIMDi *dp_h, *pre_dp_h;
    s = graph->node[node_id].base == query[max_i-1] ? match : -mismatch;
    for (k = 0; k < pre_n[index_i]; ++k) {
        pre_i = pre_index[index_i][k];
        dp_h = DP_H+index_i * dp_sn; pre_dp_h = DP_H+pre_i * dp_sn;
        _dp_h = (int16_t*)dp_h; _pre_dp_h = (int16_t*)pre_dp_h;
...
void simd_rank(abpoa_graph_t *graph, int node_id) {
    int i;
    int max_out_rank = graph->node_id_to_max_rank[node_id] + 1;
    int min_out_rank = graph->node_id_to_min_rank[node_id] + 1;
    for (i = 0; i < graph->node[node_id].out_edge_n; ++i) {
...
void simd_max(SIMDi *dp_h, SIMDi *qi, int qlen, int pn, int beg, int beg_sn, int end, int end_sn, int *max_i, int *max, SIMDi zero, SIMDi min_inf) {
    ...
    if (end_sn > beg_sn + 1) {
        a = dp_h[beg_sn+1];
        for (i = beg_sn+2; i <= end_sn-1; ++i) {
            a = SIMDMaxi16(a, dp_h[i]);
        }
        _dp_h = (int16_t*)&a;
        for (i = 0; i < 8; i++) {
            if (_dp_h[i] > *max) {
    ...
    *max = INF_16_MIN;
    ...
    a = dp_h[end_sn]; b = qi[end_sn];
    if (end_sn == qlen / pn) SIMDSetIfGreateri16(a, zero, b, min_inf, a);
    for (i = beg_sn; i <= end_sn-1; ++i) {
        SIMDGetIfGreateri16(b, a, dp_h[i], a, qi[i], b);
    }
    _dp_h = (int16_t*)&a, _qi = (int16_t*)&b;
    //print_simd(&a, "dp_h", int16_t);
    //print_simd(&b, "qi", int16_t);
    for (i = 0; i < 8; ++i) {
        if (_dp_h[i] > *max) {
            *max = _dp_h[i];
            *max_i = _qi[i];
...
//TODO for affine-gap, not maintain backtrac_z, like linear-gap
//      need store H/E/F to do backtracking
//TODO set rank only if bw >= 0
//SIMD: cal directly or use Store/Load ???
int simd16_abpoa_lg_global_align_sequence_with_graph_core(abpoa_graph_t *graph, uint8_t *query, int qlen, abpoa_para_t *abpt, int *n_cigar, abpoa_cigar_t **graph_cigar, SIMD_para_t sp) {
    if (abpt->gap_open != 0) err_fatal_simple("Gap open != 0 !\n");
    int matrix_row_n = graph->node_n, matrix_col_n = qlen + 1;
    int **pre_index, *pre_n, pre_i, best_i=0, best_j=0;
    ...
    SIMDi *DP_H, *dp_h, *pre_dp_h, *dp_f, h, e, a, b, c, *qp, *qi, gap_e, zero, min_inf;
    ...
    {   // generate the query profile
        ...
        if (abpt->use_ada) {
            qi = (SIMDi*)SIMDMalloc(dp_sn * size, size);
            int16_t *_qi = (int16_t*)qi;
            for (i = 0; i <= qlen; ++i)
                _qi[i] = i;
            for (i = qlen+1; i < (qlen/pn+1) * pn; ++i)
                _qi[i] = -1;
        }
    }
    ...
    {   // DP loop
        ...
        for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {
            ...
            simd_dp_loop(index_i, dp_beg, dp_beg_sn, dp_end, dp_end_sn, beg_sn, beg, end_sn, end, dp_sn, pn, min_inf, gap_ext, pre_n, pre_index, dp_h, DP_H, dp_f, q, gap_e);
            if (abpt->use_ada) { // set current rank, set min/max rank for next nodes
                int max_i, max;
                simd_max(dp_h, qi, qlen, pn, beg, beg_sn, end, end_sn, &max_i, &max, zero, min_inf);
#ifdef __DEBUG__
                printf("adaMax: %d, %d\n", max_i, max);
#endif
                simd_ada_rank(graph, DP_H, pre_n, pre_index, max_i, gap_ext, abpt->match, abpt->mismatch, query, node_id, index_i, dp_sn);
            } else if (abpt->bw >= 0) { // set min/max_rank for next nodes
                simd_rank(graph, node_id);
            }
        }
    }
    ...
    { // backtrack from best score
        if (n_cigar && graph_cigar) {
            simd_lg_backtrack(DP_H, dp_sn, (int16_t)abpt->match, (int16_t)abpt->mismatch, (int16_t)abpt->gap_ext, pre_index, pre_n, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar);
            //simd_abpoa_lg_backtrack(int16_t, DP_H, dp_sn, (int16_t)abpt->match, (int16_t)abpt->mismatch, (int16_t)abpt->gap_ext, pre_index, pre_n, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar);
        }
    }
