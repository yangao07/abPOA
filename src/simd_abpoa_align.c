...
void _simd_abpoa_init_var(abpoa_para_t *abpt, abpoa_graph_t *graph, uint8_t *query, int qlen, SIMDi *qp, SIMDi *qi, int *mat, int qp_sn, int pn, SIMDi SIMD_MIN_INF) {
    ...
    /* for backtrack */                                                                                                     
    // int dp_beg = abm->dp_beg, dp_end = abm->dp_end, dp_beg_sn = abm->dp_beg_sn, dp_end_sn = abm->dp_end_sn;                     
    if (abpt->bw >= 0) {                                                                                                    
        for (i = 0; i < graph->node_n; ++i) {                                                                               
            graph->node_id_to_min_rank[i] = graph->node_n;                                                                  
            graph->node_id_to_max_rank[i] = 0;                                                                              
...
void _simd_abpoa_cg_dp(SIMDi *q, SIMDi *qi, SIMDi *dp_h, SIMDi *dp_e1, SIMDi *dp_e2, int index_i, abpoa_graph_t *graph, abpoa_para_t *abpt, int dp_sn, int pn, int qlen, int w, SIMDi *DP_H2E, SIMDi *dp_f, SIMDi *dp_f2, SIMDi SIMD_MIN_INF, SIMDi zero, int inf_min, SIMDi GAP_O1, SIMDi GAP_O2, SIMDi GAP_E1, SIMDi GAP_E2, SIMDi GAP_OE1, SIMDi GAP_OE2, SIMDi* GAP_E1S, SIMDi* GAP_E2S, SIMDi *PRE_MIN, SIMDi *PRE_MASK, SIMDi *SUF_MIN, int log_n, int *dp_beg, int *dp_end, int *dp_beg_sn, int *dp_end_sn, int *pre_n, int **pre_index) {
    ...
    /* loop query */                                                                                                                
    /* init h, e */                                                                                                                 
    int _beg_sn = MAX_OF_TWO(beg_sn-1, 0), _end_sn = MIN_OF_TWO(end_sn+1, dp_sn-1); 
    for (i = _beg_sn; i <= _end_sn; ++i) { /* SIMD parallelization */                                                               
        dp_h[i] = SIMD_MIN_INF; dp_e1[i] = SIMD_MIN_INF; dp_e2[i] = SIMD_MIN_INF; dp_f[i] = SIMD_MIN_INF; dp_f2[i] = SIMD_MIN_INF;  
    }                                                                                                                               
    /* get max m and e */                                                                                                           
...
int abpoa_cg_global_align_sequence_with_graph_core(abpoa_t *ab, int qlen, uint8_t *query, abpoa_para_t *abpt, SIMD_para_t sp, abpoa_res_t *res) {
    ...
    _simd_abpoa_init_var(abpt, graph, query, qlen, qp, dp_f + dp_sn, mat, qp_sn, pn, SIMD_MIN_INF);
    /* generate the query profile */                                                                                        
    /*for (i = 0; i < qp_sn * abpt->m; ++i) qp[i] = SIMD_MIN_INF;                                                                  
    for (k = 0; k < abpt->m; ++k) { 
        int *p = &mat[k * abpt->m];                                                                                         
    ...
    if (abpt->bw >= 0 || abpt->align_mode == ABPOA_LOCAL_MODE || abpt->align_mode == ABPOA_EXTEND_MODE) {
        qi = dp_f + dp_sn; _qi = (int16_t*)qi;                                                                              
        for (i = 0; i <= qlen; ++i) _qi[i] = i;                                                                             
        for (i = qlen+1; i < (qlen/pn+1) * pn; ++i) _qi[i] = -1;                                                            
    } */                                                                                                                      
    ...
    for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {
        ...
        _simd_abpoa_cg_dp(q, qi, dp_h, dp_e1, dp_e2, index_i, graph, abpt, dp_sn, pn, qlen, w, DP_H2E, dp_f, dp_f2, SIMD_MIN_INF, zero, inf_min, GAP_O1, GAP_O2, GAP_E1, GAP_E2, GAP_OE1, GAP_OE2, GAP_E1S, GAP_E2S, PRE_MIN, PRE_MASK, SUF_MIN, log_n, dp_beg, dp_end, dp_beg_sn, dp_end_sn, pre_n, pre_index);
        ...
        for (i = 0; i < pre_n[index_i]; ++i) {    
            ...
            _beg_sn = MAX_OF_THREE(0, pre_beg_sn, (beg-1)/pn); _end_sn = MIN_OF_THREE((pre_end+1)/pn, end_sn, dp_sn-1);
            first = first_min;
            for (sn_i = _beg_sn; sn_i <= _end_sn; ++sn_i) { 
    ...
    // printf("dp_sn: %d, node_n: %d, seq_n: %d\n", tot_dp_sn, graph->node_n, qlen);   
    _simd_abpoa_global_get_max(graph, DP_H2E, dp_sn, qlen,  &best_score, &best_i, &best_j);
    /*int in_id, in_index;	                                                        
    for (i = 0; i < graph->node[ABPOA_SINK_NODE_ID].in_edge_n; ++i) {	            
        in_id = graph->node[ABPOA_SINK_NODE_ID].in_id[i];	                        
        in_index = abpoa_graph_node_id_to_index(graph, in_id);	                    
    ...
    _simd_abpoa_cg_get_cigar(abpt, graph, query, res, best_i, best_j, qlen, DP_H2E, dp_sn, pre_n, pre_index);
    /*int m = abpt->m, n_c = 0, s, m_c = 0, id, hit, cur_op = ABPOA_ALL_OP, _start_i, _start_j;
    int16_t *_pre_dp_h, *_pre_dp_e1, *_pre_dp_e2; abpoa_cigar_t *cigar = 0;
    i = best_i, j = best_j, _start_i = best_i, _start_j = best_j;
    id = abpoa_graph_index_to_node_id(graph, i);                                    
