...
#define simd_abpoa_ag_backtrack(score_t, DP_HE, dp_sn, m, mat, pre_index, pre_n, d_sn,                 \
    ...
    while (i > start_i && j > start_j) {                                                                                    \
        ...
        s = mat[m * graph->node[id].base + query[j-1]];\
        hit = 0;\
        for (k = 0; k < pre_n[i]; ++k) {\
            pre_i = pre_index_i[k];\
            /* deletion */\
...
#define simd_abpoa_ag_var(score_t, sp, SIMDSetOne) \
    ...
    int pn , qp_sn,d_sn, dp_sn; /* pn: value per SIMDi, qp_sn/dp_sn/d_sn: segmented length*/  \
    int beg, end, beg_sn, end_sn, _beg_sn, _end_sn, pre_beg_sn, pre_end, pre_end_sn, sn_i; \
    \
    SIMDi *qp, *qi=NULL, *DP_HE, *dp_h, *pre_dp_h, *dp_e, *pre_dp_e, *dp_f;                                 \
    score_t *_dp_h=NULL, *_qi, *_dp_e, *_dp_f, best_score = sp.inf_min, inf_min = sp.inf_min;                  \
...
\
{   /* allocate memory */   \
    pn = sp.num_of_value; qp_sn = d_sn = dp_sn = (matrix_col_n + pn - 1) / pn;    \
    qp = abm->s_mem;   \
...
#define simd_abpoa_ag_ada_rank(score_t) { \
    ...
    for (k = 0; k < pre_n[index_i]; ++k) {  \
        ...
        pre_dp_h = DP_HE + pre_i * 2 * dp_sn; _pre_dp_h = (score_t*)pre_dp_h; \
...
#define simd_abpoa_ag_get_cigar(score_t) {  \
    if (abpt->ret_cigar) {   \
        simd_abpoa_ag_backtrack(score_t, DP_HE, dp_sn, (score_t)abpt->m, (score_t)abpt->mat, pre_index, pre_n, d_sn, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar); \
    }   \
}

#define simd_abpoa_ag_local_get_cigar(score_t) {    \
    if (abpt->ret_cigar) {   \
        simd_abpoa_ag_local_backtrack(score_t, DP_HE, dp_sn, (score_t)abpt->m, (score_t)abpt->mat, (score_t)abpt->gap_ext, pre_index, pre_n, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar);   \
    }   \
}
...
#define simd_abpoa_ag_local_align_sequence_with_graph_core(score_t, graph, query, qlen, abpt, n_cigar, graph_cigar, sp,    \
    ...
    for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {    \
        ...
        /* set adaptive band */     \
        if (abpt->bw >= 0) { \
            if (max <= 0 || !abpt->use_ada || pre_n[index_i] == 1) {    \
                simd_abpoa_rank;    \
            } else {    /* set current rank, set min/max rank for next nodes */    \
                simd_abpoa_ag_ada_rank(score_t);    \
            }   \
...
#define simd_abpoa_lg_var(score_t, sp, SIMDSetOne) \
    ...
    SIMDi *DP_H, *dp_h, *pre_dp_h, *dp_f, *qp, *qi=NULL; \
...
#define simd_abpoa_lg_get_cigar(score_t) {	\
    if (abpt->ret_cigar) {	\
        simd_abpoa_lg_backtrack(score_t, DP_H, dp_sn, (score_t)abpt->m, (score_t)abpt->mat, (score_t)abpt->gap_ext, pre_index, pre_n, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar);	\
    }	\
}

#define simd_abpoa_lg_local_get_cigar(score_t) {	\
    if (abpt->ret_cigar) {   \
        simd_abpoa_lg_local_backtrack(score_t, DP_H, dp_sn, (score_t)abpt->m, (score_t)abpt->mat, (score_t)abpt->gap_ext, pre_index, pre_n, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar);   \
    }   \
}
...
#define simd_abpoa_lg_local_align_sequence_with_graph_core(score_t, graph, query, qlen, abpt, n_cigar, graph_cigar, sp,     \
    ...
    for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {    \
        ...
        _set_max_score(best_score, best_i, best_j, max, index_i, max_i);    \
        if (abpt->bw >= 0) { \
            if (max <= 0 || !abpt->use_ada || pre_n[index_i] == 1) { /* set min/max_rank for next nodes */  \
                simd_abpoa_rank;        \
            } else {    \
                simd_abpoa_lg_ada_rank(score_t);  /* set current rank, set min/max rank for next nodes */   \
            }   \
        }   \
    }   \
    /* simd_abpoa_print_lg_matrix(score_t); printf("best_score: (%d, %d) -> %d\n", best_i, best_j, best_score); */ \
...
void simd_abpoa_realloc(abpoa_t *ab, int qlen, abpoa_para_t *abpt, SIMD_para_t sp) {
    ...
    else s_msize += (sn * ((node_n << 1) + 1) * size); // DP_HE + dp_f
...
#ifdef __DEBUG__
void backtrack_cigar(int qlen, uint8_t *query, int *pre_n, int **pre_index, SIMDi *DP_HE, int dp_sn, abpoa_para_t *abpt, int best_i, int best_j, abpoa_graph_t *graph, int *n_cigar, abpoa_cigar_t **graph_cigar)
{                                       
    int start_i = 0, start_j = 0, m = abpt->m, *mat = abpt->mat;
    int i, j, k, pre_i, n_c = 0, s, m_c = 0, id, hit;
    SIMDi *dp_h, *pre_dp_h, *pre_dp_e;                                                                          
    int32_t *_dp_h=NULL, *_pre_dp_h, *_pre_dp_e;                                                                         
    abpoa_cigar_t *cigar = 0;                                                                              
    ...
    while (i > start_i && j > start_j) {                                                                                    
        int *pre_index_i = pre_index[i];
        dp_h = DP_HE + dp_sn * (i << 1); _dp_h = (int32_t*)dp_h; 
        s = mat[m * graph->node[id].base + query[j-1]];
        hit = 0;
        for (k = 0; k < pre_n[i]; ++k) {
            pre_i = pre_index_i[k];
            // deletion
            pre_dp_e = DP_HE + dp_sn * ((pre_i << 1) + 1); _pre_dp_e = (int32_t*)pre_dp_e;
            if (_pre_dp_e[j] == _dp_h[j]) {
                cigar = abpoa_push_cigar(&n_c, &m_c, cigar, ABPOA_CDEL, 1, id, j-1);                                              
                i = pre_i;
            ...
            // match/mismatch
            pre_dp_h = DP_HE + dp_sn * (pre_i << 1); _pre_dp_h = (int32_t*)pre_dp_h;
            if (_pre_dp_h[j-1] + s == _dp_h[j]) {
                cigar = abpoa_push_cigar(&n_c, &m_c, cigar, ABPOA_CMATCH, 1, id, j-1);                                            
                i = pre_i; --j;
                id = abpoa_graph_index_to_node_id(graph, i);
...
// 45% time
void backtrack_direction(SIMDi *backtrack_z, int index_i, int d_sn, int beg_sn, int end_sn, SIMDi *hd, SIMDi *dp_h, SIMDi *dp_e, SIMDi *dp_f, SIMDi *q, SIMDi he, SIMDi hm, SIMDi hf, SIMDi ee, SIMDi em, SIMDi ff, SIMDi fm, int beg, int end, int32_t *_dp_h, int32_t *_dp_f, SIMDi gap_oe, SIMDi gap_e, int gap_OE, int gap_ext) {
    SIMDi *z = backtrack_z + (index_i-1) * d_sn, tmp_e, ed, fd;
    int sn_i, q_i;
    /* compare M, E, and F */   
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { /* SIMD parallelization */
        SIMDGetIfGreateri32(hd[sn_i], dp_h[sn_i], dp_e[sn_i], SIMDAddi32(dp_h[sn_i], q[sn_i]), he, hm);    
    }   
    /* set F from (index_i, q_i-1) */   
    for (q_i = beg+1; q_i <= end; ++q_i) { /* no SIMD parallelization */    
        _dp_f[q_i] = MAX_OF_TWO(_dp_h[q_i-1] - gap_OE, _dp_f[q_i-1] - gap_ext); 
    ...
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { /* SIMD parallelization */
        ...
        SIMDGetIfGreateri32(hd[sn_i], dp_h[sn_i], dp_f[sn_i], dp_h[sn_i], hf, hd[sn_i]); 
        /* e, ed for next cell */   
        tmp_e = SIMDSubi32(dp_h[sn_i], gap_oe);  
        SIMDGetIfGreateri32(ed, dp_e[sn_i], SIMDSubi32(dp_e[sn_i], gap_e), tmp_e, ee, em);   
        /* fd for next cell */  
        SIMDSetIfGreateri32(fd, SIMDSubi32(dp_f[sn_i], gap_e), tmp_e, ff, fm);  
        z[sn_i] = SIMDOri(SIMDOri(hd[sn_i], ed), fd);  
    }
}
void set_M_E_F(int beg_sn, int end_sn, SIMDi *dp_h, SIMDi *q, SIMDi *dp_e, int *_dp_f, int beg, int end, int *_dp_h, int gap_OE, int gap_ext, SIMDi *dp_f, SIMDi gap_e, SIMDi gap_oe, int inf_min) {    
    int sn_i, q_i;
    /* compare M, E, and F */   
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { /* SIMD parallelization */
        dp_h[sn_i] = SIMDMaxi32(SIMDAddi32(dp_h[sn_i], q[sn_i]), dp_e[sn_i]);  
    }   
    /* set F from (index_i, q_i-1) */   
    _dp_f[beg] = inf_min;   
    for (q_i = beg+1; q_i <= end; ++q_i) { /* no SIMD parallelization */
    ...
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { /* SIMD parallelization */    
        /* h for current cell */ 
        dp_h[sn_i] = SIMDMaxi32(dp_h[sn_i], dp_f[sn_i]); 
        /* e for next cell */   
        dp_e[sn_i] = SIMDMaxi32(SIMDSubi32(dp_e[sn_i], gap_e), SIMDSubi32(dp_h[sn_i], gap_oe));  
    }   
}

// 20% time
void get_max_m_e(SIMDi min_inf, int pn, int beg, int beg_sn, int end_sn, int index_i, int *pre_n, int **pre_index, SIMDi *DP_HE, SIMDi *dp_h, SIMDi *dp_e, int dp_sn, int *dp_beg, int *dp_end, int *dp_beg_sn, int *dp_end_sn) {
    int i, pre_i, pre_beg_sn, pre_end, pre_end_sn, _beg_sn, _end_sn, sn_i;
    SIMDi *pre_dp_h, *pre_dp_e;
    for (i = 0; i < pre_n[index_i]; ++i) {  
        pre_i = pre_index[index_i][i];  
        ...
        _beg_sn = MAX_OF_THREE(0, pre_beg_sn, (beg-1)/pn);  
        _end_sn = MIN_OF_THREE((pre_end+1)/pn, end_sn, dp_sn-1);    
        SIMDi first = SIMDShiftRight(min_inf, SIMDTotalBytes-SIMDShiftOneNi32); 
        for (sn_i = _beg_sn; sn_i <= _end_sn; ++sn_i) { /* SIMD parallelization */ 
            SIMDi remain = SIMDShiftLeft(pre_dp_h[sn_i], SIMDShiftOneNi32);  
        ...
        _beg_sn = MAX_OF_TWO(pre_beg_sn, beg_sn); _end_sn = MIN_OF_TWO(pre_end_sn, end_sn); 
        for (sn_i = _beg_sn; sn_i <= _end_sn; ++sn_i) { /* SIMD parallelization */  
            dp_e[sn_i] = SIMDMaxi32(pre_dp_e[sn_i], dp_e[sn_i]);  
        }   
    }
}

// 30% time
void simd_abpoa_32_ag_global_align_sequence_with_graph_core(abpoa_t *ab, uint8_t *query, int qlen, abpoa_para_t *abpt, int *n_cigar, abpoa_cigar_t **graph_cigar, SIMD_para_t sp, int *b_score) {
    /* simd_abpoa_ag_var(int32_t, sp, SIMDSetOne)*/
    abpoa_graph_t *graph = ab->abg; abpoa_simd_matrix_t *abm = ab->abm;           
    int **pre_index, *pre_n, pre_i; 
    int matrix_row_n = graph->node_n, matrix_col_n = qlen + 1;   
    int i, j, k, *dp_beg, *dp_beg_sn, *dp_end, *dp_end_sn, node_id, index_i, q_i;    
    ...
    SIMDi *qp, *qi=NULL;                                                        
    SIMDi *DP_HE, *dp_h, *pre_dp_h, *dp_e, *pre_dp_e, *dp_f;                                 

    int32_t *_dp_h=NULL, *_qi, *_dp_e, *_dp_f, best_score = sp.inf_min, inf_min = sp.inf_min;                  
    int *mat = abpt->mat, best_i = 0, best_j = 0, max, max_i;                                       
    int32_t gap_open = abpt->gap_open, gap_ext = abpt->gap_ext, gap_OE = abpt->gap_open + abpt->gap_ext;    

    int w = abpt->bw < 0 ? qlen : abpt->bw; /* when w < 0, do whole global */   
    SIMDi zero = SIMDSetZeroi(), min_inf = SIMDSetOnei32(inf_min), gap_e = SIMDSetOnei32(gap_ext), gap_oe = SIMDSetOnei32(gap_OE); 
    ...
    {   /* allocate memory */
        ...
        for (k = 0; k < abpt->m; ++k) { /* SIMD parallelization */ 
            int *p = &mat[k * abpt->m]; 
            int32_t *_qp = (int32_t*)(qp + k * qp_sn);  
            _qp[0] = 0;     
    ...
    /*simd_abpoa_ag_first_dp(int32_t)*/
    {
        /*simd_abpoa_ag_first_row(int32_t)*/ 
        {
            /* fill the first row */
            if (abpt->bw >= 0) {    
                graph->node_id_to_min_rank[0] = graph->node_id_to_max_rank[0] = 0;  
            ...
            dp_beg_sn[0] = (dp_beg[0])/pn; dp_end_sn[0] = (dp_end[0])/pn;   
            dp_h = DP_HE, dp_e = dp_h + dp_sn;  
            _dp_h = (int32_t*)dp_h, _dp_e = (int32_t*)dp_e; 
            _end_sn = MIN_OF_TWO(dp_end_sn[0]+1, dp_sn-1);  
        }
        for (i = 0; i <= _end_sn; ++i) {    
            dp_h[i] = min_inf; dp_e[i] = min_inf;   
    ...
    int sn_n = 0;
    for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {    
        /*simd_abpoa_ag_dp(int32_t, SIMDShiftOneN, SIMDMax, SIMDAdd, SIMDSub, SIMDGetIfGreater, SIMDSetIfGreater)*/
        { 
            node_id = abpoa_graph_index_to_node_id(graph, index_i); 
            SIMDi *q = qp + graph->node[node_id].base * qp_sn;  
            dp_h = DP_HE + index_i * 2 * dp_sn, dp_e = dp_h + dp_sn;    
            _dp_h = (int32_t*)dp_h, _dp_e = (int32_t*)dp_e, _dp_f = (int32_t*)dp_f; 
            if (abpt->bw < 0) beg = dp_beg[index_i] = 0, end = dp_end[index_i] = qlen;  
            else beg = dp_beg[index_i] = GET_DP_BEGIN(graph, w, node_id, qlen), end = dp_end[index_i] = GET_DP_END(graph, w, node_id, qlen);    
            beg_sn = dp_beg_sn[index_i] = (dp_beg[index_i])/pn; end_sn = dp_end_sn[index_i] = (dp_end[index_i])/pn; 
            sn_n += (end_sn - beg_sn + 1);
            /* loop query */    
            /* init h, e */     
            ...
            /* get max m and e */   
            get_max_m_e(min_inf, pn, beg, beg_sn, end_sn, index_i, pre_n, pre_index, DP_HE, dp_h, dp_e, dp_sn, dp_beg, dp_end, dp_beg_sn, dp_end_sn);
            set_M_E_F(beg_sn, end_sn, dp_h, q, dp_e, _dp_f, beg, end, _dp_h, gap_OE, gap_ext, dp_f, gap_e, gap_oe, inf_min);
        }
        if (abpt->bw >= 0) {    
            if (!abpt->use_ada || pre_n[index_i] == 1) { /* set current rank, set min/max rank for next nodes */   
                //simd_abpoa_rank;    
            ...
            } else { /* use_ada && pre_n > 1 */
                /*simd_abpoa_max(int32_t, SIMDSetIfGreater, SIMDGetIfGreater)*/
                /* select max dp_h */   
                max = inf_min, max_i = -1;   
                SIMDi a = dp_h[end_sn], b = qi[end_sn];   
                if (end_sn == qlen / pn) SIMDSetIfGreateri32(a, zero, b, min_inf, a);  
                for (i = beg_sn; i < end_sn; ++i) { 
                    SIMDGetIfGreateri32(b, a, dp_h[i], a, qi[i], b);   
                }   
                _dp_h = (int32_t*)&a, _qi = (int32_t*)&b; 
                for (i = 0; i < pn; ++i) {  
                    if (_dp_h[i] > max) {   
                        max = _dp_h[i]; 
                        max_i = _qi[i];  
                ...
                /*printf("adaMax: %d, %d\n", max_i, max);*/ 
                /*simd_abpoa_ag_ada_rank(int32_t)*/
                /* determine max pre_i, then determin current rank */   
                int s, max_pre_i=-1; int32_t *_pre_dp_h, *_pre_dp_e;    
                s = abpt->mat[abpt->m * graph->node[node_id].base + query[max_i-1]];    
                dp_h = DP_HE + index_i * 2 * dp_sn; _dp_h = (int32_t*)dp_h;
                for (k = 0; k < pre_n[index_i]; ++k) {
                    pre_i = pre_index[index_i][k];
                    pre_dp_h = DP_HE + pre_i * 2 * dp_sn; _pre_dp_h = (int32_t*)pre_dp_h;
                    if (_pre_dp_h[max_i-1] + s == _dp_h[max_i]) {   
                        max_pre_i = pre_i;  
                        break;  
                    }
                    pre_dp_e = DP_HE + (pre_i * 2 + 1) * dp_sn; _pre_dp_e = (int32_t*)pre_dp_e;
                    if (_pre_dp_e[max_i] == _dp_h[max_i]) {
                        max_pre_i = pre_i;
                        break;
                    }
                }
                int max_pre_id = abpoa_graph_index_to_node_id(graph, max_pre_i);
                /*simd_abpoa_ada_rank_from_maxpre*/
                /* set min/max_rank for next nodes */ 
                int pre_max_rank, pre_min_rank; 
                pre_max_rank = graph->node_id_to_max_rank[max_pre_id];
    ...
    /*simd_abpoa_ag_global_get_max(int32_t)*/
    {	
        int in_id, in_index;	
        for (i = 0; i < graph->node[ABPOA_SINK_NODE_ID].in_edge_n; ++i) { 	
            in_id = graph->node[ABPOA_SINK_NODE_ID].in_id[i];	
    ...
    /* simd_abpoa_print_ag_matrix(int32_t);
       printf("best_score: (%d, %d) -> %d\n", best_i, best_j, best_score); */ 
    /*simd_abpoa_ag_get_cigar(int32_t) */
    {  
        if (abpt->ret_cigar) {   
            /*simd_abpoa_ag_backtrack(int32_t, DP_HE, dp_sn, (int32_t)abpt->m, (int32_t)abpt->mat, (int32_t)abpt->gap_ext, pre_index, pre_n, backtrack_z, d_sn, 0, 0, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar); \*/
            /*simd_abpoa_ag_backtrack(int32_t, DP_HE, dp_sn, m, mat, gap_e, pre_index, pre_n, backtrack_z, d_sn, start_i, start_j, best_i, best_j, qlen, graph, query, n_cigar, graph_cigar)*/ 
            backtrack_cigar(qlen, query, pre_n, pre_index, DP_HE, dp_sn, abpt, best_i, best_j, graph, n_cigar, graph_cigar);
            //{                                       
            //    int start_i = 0, start_j = 0, m = abpt->m, *mat = abpt->mat, gap_e = abpt->gap_ext;
            //    int i, j, k, pre_i;                                                                                                     
            //    SIMDi *dp_h, *pre_dp_h, *dp_e, *pre_dp_e, *bz;                                                                          
            //    int32_t *_dp_h=NULL, *_pre_dp_h, *_dp_e, *_pre_dp_e;                                                                         
            //    int n_c = 0, s, m_c = 0, id, which, last_which;                                                                         
            //    int op_shift[3] = {HOP_OFF_SET, EOP_OFF_SET, FOP_OFF_SET};                                                              
            //    int32_t d, *_bz; abpoa_cigar_t *cigar = 0;                                                                              
            ...
            //                _dp_h = (int32_t*)dp_h; _pre_dp_h = (int32_t*)pre_dp_h;                                                     
            //                if (_pre_dp_h[j-1] + s == _dp_h[j]) {                                                                       
            //                    i = pre_i;                                                                                              
            //                    break;                                                                                                  
            //                }                                                                                                           
            ...
            //                    _dp_e = (int32_t*)dp_e; _pre_dp_e = (int32_t*)pre_dp_e;                                                 
            ...
            //                    _dp_h = (int32_t*)dp_h; _pre_dp_e = (int32_t*)pre_dp_e;                                                 
            //                    if (_pre_dp_e[j] == _dp_h[j]) {                                                                         
            //                        i = pre_i;                                                                                          
            //                        break;                                                                                              
            //                    }                                                                                                       
...
int simd_abpoa_align_sequence_with_graph(abpoa_t *ab, uint8_t *query, int qlen, abpoa_para_t *abpt, int *n_cigar, abpoa_cigar_t **graph_cigar) { // global, 32bits, affine-gap/linear-gap
    int _best_score = 0;
    ...
    if (abpt->gap_open > 0) {
        simd_abpoa_32_ag_global_align_sequence_with_graph_core(ab, query, qlen, abpt, n_cigar, graph_cigar, _simd_p32, &_best_score);
    } else {
        // simd_abpoa_32_lg_global_align_sequence_with_graph_core(ab, query, qlen, abpt, n_cigar, graph_cigar, _simd_p32, &_best_score);
    }
    return _best_score;
}
#else
