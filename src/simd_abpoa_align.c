...
#define debug_simd_abpoa_print_cg_matrix_row(str, score_t, index_i) {                           \
    ...
    for (i = dp_beg[index_i]; i <= (dp_end[index_i]/16+1)*16-1; ++i) {	                        \
        printf("%d:(%d,%d,%d,%d,%d)\t", i, _dp_h[i], _dp_e1[i],_dp_e2[i], _dp_f1[i],_dp_f2[i]); \
    } printf("\n");	                                                                            \
}
...
#define simd_abpoa_print_cg_matrix(score_t) {                                                       \
    for (j = 0; j <= matrix_row_n-2; ++j) {	                                                        \
        ...
        score_t *_dp_h=(score_t*)dp_h, *_dp_e1=(score_t*)dp_e1, *_dp_e2=(score_t*)dp_e2, *_dp_f1=(score_t*)dp_f1, *_dp_f2=(score_t*)dp_f2;	\
        for (i = dp_beg[j]; i <= dp_end[j]; ++i) {                                                  \
            printf("%d:(%d,%d,%d,%d,%d)\t", i, _dp_h[i], _dp_e1[i],_dp_e2[i], _dp_f1[i],_dp_f2[i]);	\
        } printf("\n");	                                                                            \
    }	                                                                                            \
...
#define simd_abpoa_ag_backtrack(score_t, DP_HEF, dp_sn, m, mat, gap_e, pre_index, pre_n,                  \
            ...
            for (k = 0; k < pre_n[i]; ++k) {                                                                \
                ...
                if (cur_op & ABPOA_M_OP) {                                                                  \
                    if (_dp_h[j] == _pre_dp_e1[j]) { /* deletion */                                         \
                        _pre_dp_h = (score_t*)(DP_HEF + dp_sn * pre_i * 3);                                 \
                        if (_pre_dp_h[j] - gap_e == _pre_dp_e1[j]) cur_op = ABPOA_E1_OP;                    \
...
#define simd_abpoa_ag_get_cigar(score_t) {                                      \
    if (abpt->ret_cigar) {                                                      \
        simd_abpoa_ag_backtrack(score_t, DP_HEF, dp_sn, (score_t)abpt->m,        \
            (score_t)abpt->mat, gap_ext1, pre_index, pre_n, 0, 0,               \
            best_i, best_j, qlen, graph, query, res);                           \
    }                                                                           \
} 
...
#define simd_abpoa_cg_get_cigar(score_t) {                                      \
    if (abpt->ret_cigar) {                                                      \
        simd_abpoa_cg_backtrack(score_t, DP_H2E2F, dp_sn, (score_t)abpt->m,       \
...
#define simd_abpoa_ag_global_align_sequence_to_graph_core(score_t, ab, query, qlen, abpt, res, sp,                  \
    ...
    simd_abpoa_global_get_max(score_t, DP_HEF, 3*dp_sn);                                                             \
...
#define simd_abpoa_cg_global_align_sequence_to_graph_core(score_t, ab, query, qlen, abpt, res, sp,              \
 ...
 /* printf("dp_sn: %d\n", tot_dp_sn); */                                                                        \
    simd_abpoa_global_get_max(score_t, DP_H2E2F, 5*dp_sn);                                                      \
...
void abpoa_cg_first_dp(abpoa_para_t *abpt, abpoa_graph_t *graph, int *dp_beg, int *dp_end, int *dp_beg_sn, int *dp_end_sn, int pn, int qlen, int w, int dp_sn, SIMDi *DP_H2E2F, SIMDi SIMD_INF_MIN, int16_t inf_min, int gap_open1, int gap_ext1, int gap_open2, int gap_ext2, int gap_oe1, int gap_oe2) {
    int i, _end_sn;
    if (abpt->bw >= 0) {                                                                                
        graph->node_id_to_min_rank[0] = graph->node_id_to_max_rank[0] = 0;                              
        for (i = 0; i < graph->node[0].out_edge_n; ++i) { /* set min/max rank for next_id */            
    ...
    int16_t *_dp_h = (int16_t*)dp_h, *_dp_e1 = (int16_t*)dp_e1, *_dp_e2 = (int16_t*)dp_e2, *_dp_f1 = (int16_t*)dp_f1, *_dp_f2 = (int16_t*)dp_f2;
    _dp_h[0] = 0; _dp_e1[0] = -(gap_oe1); _dp_e2[0] = -(gap_oe2); _dp_f1[0] = _dp_f2[0] = inf_min;
    for (i = 1; i <= dp_end[0]; ++i) { /* no SIMD parallelization */                
        _dp_f1[i] = -(gap_open1 + gap_ext1 * i);
        _dp_f2[i] = -(gap_open2 + gap_ext2 * i);
...
void abpoa_global_get_max(abpoa_graph_t *graph, SIMDi *DP_H_HE, int dp_sn, int qlen, int16_t *best_score, int *best_i, int *best_j) {
...
int abpoa_cg_dp(SIMDi *q, SIMDi *dp_h, SIMDi *dp_e1, SIMDi *dp_e2, SIMDi *dp_f1, SIMDi *dp_f2, int index_i, abpoa_graph_t *graph, abpoa_para_t *abpt, int dp_sn, int pn, int qlen, int w, SIMDi *DP_H2E2F, SIMDi SIMD_INF_MIN, SIMDi GAP_O1, SIMDi GAP_O2, SIMDi GAP_E1, SIMDi GAP_E2, SIMDi GAP_OE1, SIMDi GAP_OE2, SIMDi* GAP_E1S, SIMDi* GAP_E2S, SIMDi *PRE_MIN, SIMDi *PRE_MASK, SIMDi *SUF_MIN, int log_n, int *dp_beg, int *dp_end, int *dp_beg_sn, int *dp_end_sn, int *pre_n, int **pre_index) {
    ...
    else _beg_sn = pre_beg_sn, first = SIMDShiftRight(SIMD_INF_MIN, SIMDTotalBytes-SIMDShiftOneNi16);
    _end_sn = MIN_OF_THREE((pre_end+1)/pn, end_sn, dp_sn-1);
    for (i = beg_sn; i < _beg_sn; ++i) dp_h[i] = SIMD_INF_MIN;
    for (i = _end_sn+1; i <= MIN_OF_TWO(end_sn+1, dp_sn-1); ++i) dp_h[i] = SIMD_INF_MIN;
    for (sn_i = _beg_sn; sn_i <= _end_sn; ++sn_i) { /* SIMD parallelization */
    ...
    // if (index_i == 182) debug_simd_abpoa_print_cg_matrix_row("1", int16_t, index_i);
    // new init end
    /* get max m and e */
    for (i = 1; i < pre_n[index_i]; ++i) {
        pre_i = pre_index[index_i][i];
    ...
    // debug_simd_abpoa_print_cg_matrix_row("2", int16_t, index_i);
    /* compare M, E, and F */
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { /* SIMD parallelization */
        dp_h[sn_i] =  SIMDMaxi16(SIMDMaxi16(SIMDAddi16(dp_h[sn_i], q[sn_i]), dp_e1[sn_i]), dp_e2[sn_i]);
    }
    // debug_simd_abpoa_print_cg_matrix_row("3", int16_t, index_i);
    /* new F start */
    ...
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) {
        ...
        /* F = (H << 1 | x) - OE */
        // debug_simd_abpoa_print_cg_matrix_row("4.1", int16_t, index_i);
        dp_f1[sn_i] = SIMDSubi16(SIMDOri(SIMDShiftLeft(dp_h[sn_i], SIMDShiftOneNi16), first), GAP_OE1);
        dp_f2[sn_i] = SIMDSubi16(SIMDOri(SIMDShiftLeft(dp_h[sn_i], SIMDShiftOneNi16), first2), GAP_OE2);
        /* F = max{F, (F-e)<<1}, F = max{F, (F-2e)<<2} ... */
        // debug_simd_abpoa_print_cg_matrix_row("4.2", int16_t, index_i);
        SIMD_SET_F(dp_f1[sn_i], log_n, set_num, PRE_MIN, PRE_MASK, SUF_MIN, GAP_E1S, SIMDMaxi16, SIMDAddi16, SIMDSubi16, SIMDShiftOneNi16);
        SIMD_SET_F(dp_f2[sn_i], log_n, set_num, PRE_MIN, PRE_MASK, SUF_MIN, GAP_E2S, SIMDMaxi16, SIMDAddi16, SIMDSubi16, SIMDShiftOneNi16);
        /* x = max{H, F+o} */
        // debug_simd_abpoa_print_cg_matrix_row("4.3", int16_t, index_i);
        ...
        // debug_simd_abpoa_print_cg_matrix_row("4.4", int16_t, index_i);
    }
    debug_simd_abpoa_print_cg_matrix_row("4", int16_t, index_i);
    for (sn_i = beg_sn; sn_i <= end_sn; ++sn_i) { /* SIMD parallelization */
        /* e for next cell */
    ...
    // if (index_i == 182) debug_simd_abpoa_print_cg_matrix_row("5", int16_t, index_i);
    return tot_dp_sn;
}

void abpoa_cg_backtrack(SIMDi *DP_H2E2F, int *dp_beg, int *dp_end, int dp_sn, int m, int *mat, int gap_ext1, int gap_ext2, int gap_oe1, int gap_oe2, int **pre_index, int *pre_n, int start_i, int start_j, int best_i, int best_j, int qlen, abpoa_graph_t *graph, abpoa_para_t *abpt, uint8_t *query, abpoa_res_t *res) {
    int i, j, k, pre_i, n_c = 0, s, m_c = 0, id, hit, cur_op = ABPOA_ALL_OP, _start_i, _start_j;            
    SIMDi *dp_h, *dp_e1, *dp_e2, *pre_dp_h, *pre_dp_e1, *pre_dp_e2, *dp_f1, *dp_f2;                                                        
    int16_t *_dp_h=NULL, *_dp_e1, *_dp_e2, *_pre_dp_h, *_pre_dp_e1, *_pre_dp_e2, *_dp_f1, *_dp_f2; abpoa_cigar_t *cigar = 0;                    
    i = best_i, j = best_j, _start_i = best_i, _start_j = best_j;                                           
    id = abpoa_graph_index_to_node_id(graph, i);                                                            
    if (best_j < qlen) cigar = abpoa_push_cigar(&n_c, &m_c, cigar, ABPOA_CSOFT_CLIP, qlen-j, -1, qlen-1);   
    dp_h = DP_H2E2F + dp_sn * (i * 5); _dp_h = (int16_t*)dp_h;
    while (i > start_i && j > start_j) {                                                                    
        _start_i = i, _start_j = j;                                                                         
        int *pre_index_i = pre_index[i];                                                                    
        s = mat[m * graph->node[id].base + query[j-1]]; hit = 0;                                            
        if (cur_op & ABPOA_M_OP) {                                                                          
            for (k = 0; k < pre_n[i]; ++k) {                                                                
                pre_i = pre_index_i[k];                                                                     
                if (j-1 < dp_beg[pre_i] || j-1 > dp_end[pre_i]) continue;                                       \
                pre_dp_h = DP_H2E2F + dp_sn * (pre_i * 5); _pre_dp_h = (int16_t*)pre_dp_h;                    
                if (_pre_dp_h[j-1] + s == _dp_h[j]) { /* match/mismatch */                                  
                    cigar = abpoa_push_cigar(&n_c, &m_c, cigar, ABPOA_CMATCH, 1, id, j-1);                  
        ...
        if (hit == 0 && cur_op & ABPOA_E_OP) {                                                              
            ...
            _dp_e1 = (int16_t*)dp_e1, _dp_e2 = (int16_t*)dp_e2;
            for (k = 0; k < pre_n[i]; ++k) {                                                                
                pre_i = pre_index_i[k];                                                                     
                if (j < dp_beg[pre_i] || j > dp_end[pre_i]) continue;
                pre_dp_h = DP_H2E2F + dp_sn * (pre_i * 5); _pre_dp_h = (int16_t*)pre_dp_h;            
                if (cur_op & ABPOA_E1_OP) {                                                                 
                    pre_dp_e1 = DP_H2E2F + dp_sn * ((pre_i * 5) + 1); _pre_dp_e1 = (int16_t*)pre_dp_e1;       
                    if (cur_op & ABPOA_M_OP) {
                        if (_dp_h[j] == _pre_dp_e1[j]) { 
                            if (_pre_dp_h[j] - gap_oe1 == _pre_dp_e1[j]) cur_op = ABPOA_M_OP;
                            else cur_op = ABPOA_E1_OP;
                ...
                if (cur_op & ABPOA_E2_OP) {                                                                 
                    pre_dp_e2 = DP_H2E2F + dp_sn * ((pre_i * 5) + 2); _pre_dp_e2 = (int16_t*)pre_dp_e2;       
                    if (cur_op & ABPOA_M_OP) {
                        if (_dp_h[j] == _pre_dp_e2[j]) {
                            if (_pre_dp_h[j] - gap_oe2 == _pre_dp_e2[j]) cur_op = ABPOA_M_OP;
                            else cur_op = ABPOA_E2_OP;
        ...
        if (hit == 0 && cur_op & ABPOA_F_OP) {
            if (cur_op & ABPOA_F1_OP) {
                dp_f1 = dp_h + dp_sn * 3; _dp_f1 = (int16_t*)dp_f1;
                if (_dp_h[j-1] - gap_oe1 == _dp_f1[j]) {
                    cur_op = ABPOA_M_OP;
                    hit = 1;
                } else if (_dp_f1[j-1] - gap_ext1 == _dp_f1[j]) {
                    cur_op = ABPOA_F1_OP;
                    hit = 1;
                }
            } 
            if (hit == 0 && cur_op & ABPOA_F2_OP) {
                dp_f2 = dp_h + dp_sn * 4; _dp_f2 = (int16_t*)dp_f2;
                if (_dp_h[j-1] - gap_oe2 == _dp_f2[j]) cur_op = ABPOA_M_OP;
                else if (_dp_f2[j-1] - gap_ext2 == _dp_f2[j]) cur_op = ABPOA_F2_OP;
                else exit(1);
            }
            cigar = abpoa_push_cigar(&n_c, &m_c, cigar, ABPOA_CINS, 1, id, j-1); --j;                       
            ++res->n_aln_bases;                                                                             
...
int abpoa_cg_global_align_sequence_to_graph_core(abpoa_t *ab, int qlen, uint8_t *query, abpoa_para_t *abpt, SIMD_para_t sp, abpoa_res_t *res) {
    ...
    int w = abpt->bw < 0 ? qlen : abpt->bw; /* when w < 0, do whole global */                       
    SIMDi zero = SIMDSetZeroi(), SIMD_INF_MIN = SIMDSetOnei16(inf_min);                                     
    pn = sp.num_of_value; qp_sn = dp_sn = (matrix_col_n + pn - 1) / pn, log_n = sp.log_num, size = sp.size; 
    qp = abm->s_mem;                                                                                

    // simd_abpoa_cg_only_var(int16_t, SIMDSetOne); // ag_only_var
    ...
    SUF_MIN[0] = SIMDShiftLeft(SIMD_INF_MIN, SIMDShiftOneNi16); 
    ...
    for (index_i = 1; index_i < matrix_row_n-1; ++index_i) {
        ...
        dp_h = DP_H2E2F + (index_i*5) * dp_sn; dp_e1 = dp_h + dp_sn; dp_e2 = dp_e1 + dp_sn; dp_f1 = dp_e2 + dp_sn; dp_f2 = dp_f1 + dp_sn; 
        tot_dp_sn += abpoa_cg_dp(q, dp_h, dp_e1, dp_e2, dp_f1, dp_f2, index_i, graph, abpt, dp_sn, pn, qlen, w, DP_H2E2F, SIMD_INF_MIN, GAP_O1, GAP_O2, GAP_E1, GAP_E2, GAP_OE1, GAP_OE2, GAP_E1S, GAP_E2S, PRE_MIN, PRE_MASK, SUF_MIN, log_n, dp_beg, dp_end, dp_beg_sn, dp_end_sn, pre_n, pre_index);
        if (abpt->bw >= 0) {
    ...
    abpoa_global_get_max(graph, DP_H2E2F, 5*dp_sn, qlen,  &best_score, &best_i, &best_j);
 // simd_abpoa_print_cg_matrix(int16_t); printf("best_score: (%d, %d) -> %d\n", best_i, best_j, best_score);        
    abpoa_cg_backtrack(DP_H2E2F, dp_beg, dp_end, dp_sn, abpt->m, mat, gap_ext1, gap_ext2, gap_oe1, gap_oe2, pre_index, pre_n, 0, 0, best_i, best_j, qlen, graph, abpt, query, res);
    simd_abpoa_free_var; free(GAP_E1S); free(GAP_E2S);
    return best_score;
}
