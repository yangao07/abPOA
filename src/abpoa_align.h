...
// with w: (min-w, max+w)
#define GET_DP_BEGIN(graph, w, i) MAX_OF_TWO(0, MIN_OF_TWO(abpoa_graph_node_id_to_min_rank(graph, i), qlen - abpoa_graph_node_id_to_max_remain(graph, i))-w)
#define GET_DP_END(graph, w, i) MIN_OF_TWO(qlen, MAX_OF_TWO(abpoa_graph_node_id_to_max_rank(graph, i), qlen - abpoa_graph_node_id_to_min_remain(graph, i))+w)
